...
import { Target, Layers, AlertTriangle, Star, CheckSquare, Sparkles, ChevronDown, ChevronUp, Briefcase } from 'lucide-react';
...
const SectionHeader = ({ id, icon: Icon, title, color, isExpanded, onToggle }: {
    ...
    return (
        ...
        >
            ...
            {isExpanded ? <ChevronUp size={16} className="text-white/50" /> : <ChevronDown size={16} className="text-white/30" />}
        </button>
    );
};
...
const GlassTips: React.FC<GlassTipsProps> = ({ tips, className }) => {
    if (!tips) return (
        <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-center p-6 space-y-4">
            ...
            <div>
                <p className="text-sm font-medium text-cyan-200 animate-pulse">Creating custom tips for this question...</p>
    ...
    return (
        <div className={cn("w-full h-full overflow-y-auto pr-2 custom-scrollbar text-left", className)}>
            <div className="space-y-1 pb-6">
                ...
                <SectionHeader id="pointsToCover" icon={CheckSquare} title="Points to Cover" color="emerald" isExpanded={expandedSection === 'pointsToCover'} onToggle={toggleSection} />
                <AnimatePresence>
                    ...
                    {expandedSection === 'industry' && (
                        ...
                        >
                            <div className="p-4 bg-black/20 border border-white/5 rounded-lg mb-3 space-y-4 text-left">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Recommended Tools</p>
                                    <p className="text-gray-300 font-medium text-sm">{decodeHtml(tips.industrySpecifics.tools)}</p>
                                </div>
                            </div>
                        </motion.div>
                    )}
                ...
                <SectionHeader id="mistakes" icon={AlertTriangle} title="Mistakes to Avoid" color="rose" isExpanded={expandedSection === 'mistakes'} onToggle={toggleSection} />
                <AnimatePresence>
                ...
                {/* Expert Pro Tip */}
                <SectionHeader id="protip" icon={Star} title="Pro Tip" color="amber" isExpanded={expandedSection === 'protip'} onToggle={toggleSection} />
                <AnimatePresence>
